...
namespace MVCManukauTech.Controllers
{
    public class OrderDetailsController : Controller
    {
        ...
        public ActionResult ShoppingCart()
        {
            ...
            if (User.IsInRole("MemberAssociate"))
            {
                //Code for action for a Full Member
                //originalGrandTotal = grandTotal;
                grandTotal = (88 * grandTotal) / 100;
                ViewBag.Discount = grandTotal.ToString();

            }
            else if (User.IsInRole("MemberFull"))
            {
                //originalGrandTotal = grandTotal;
                grandTotal = (92 * grandTotal) / 100;
        ...
        public async Task<ActionResult> Checkout()
        {
            ...
            decimal originalGrandTotal = 0;
            //Also persist because we will need it later for the "Bank of Fiction", or other Credit Card Authority
            if (null != this.User && null != this.User.Identity && !string.IsNullOrWhiteSpace(this.User.Identity.GetUserId()))
            {
                var loggedInUserId = this.User.Identity.GetUserId();
                var userManager = await this.UserManager.FindByIdAsync(loggedInUserId);
                var role = userManager.Roles?.FirstOrDefault();
                if (role != null)
                {
                    var roleInfo = await this.RoleManager.FindByIdAsync(role.RoleId);
                    if (null != roleInfo)
                    {
                        if (roleInfo.Name == "MemberFull")
                        {
                            originalGrandTotal = grandTotal;
                            grandTotal = (92 * grandTotal) / 100;
                        }
                        else if (roleInfo.Name == "MemberAssociate")
                        {
                            originalGrandTotal = grandTotal;
                            grandTotal = (88 * grandTotal) / 100;
                        }
            ...
            {
                Session["OriginalGrandTotal"] = grandTotal;
            ...
            Session["OriginalGrandTotal"] = originalGrandTotal;
            Session["GrandTotal"] = grandTotal;
            return View(checkout);
        }
